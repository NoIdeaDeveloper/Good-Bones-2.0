...
def cache_bust_for(output_file: Path) -> str:
    ...
    The hash covers only the static assets the page actually references, so
    a change to blog.css bumps blog pages without needlessly invalidating
    the homepage (and vice versa). Minified bytes are hashed so content
    changes are still reflected even when minification is enabled.
    """
    tracked = {
    ...
    if rel.parts[0] == "blog":
        used = ["script.min.js", "css/global.min.css", "css/blog.min.css"]
    elif rel.name == "index.html":
        used = ["script.min.js", "css/global.min.css", "css/index.min.css"]
    else:
        used = ["script.min.js", "css/global.min.css"]
    combined = b""
    for logical in used:
        path = tracked.get(ROOT / logical)
...
def build_home_index(contact: dict, posts: list[dict]) -> None:
    ...
    if faq_data.get("items"):
        faq_items = "\n\n".join(
            ...
            f'            <button class="faq__question" type="button" aria-expanded="false" aria-controls="faq-answer-{idx}" id="faq-question-{idx}">\n'
            f'              <span class="faq__question-text">{item["question"]}</span>\n'
            f'              <span class="faq__icon" aria-hidden="true"><span class="faq__icon-line faq__icon-line--h"></span><span class="faq__icon-line faq__icon-line--v"></span></span>\n'
            f'            </button>\n'
            f'          </h3>\n'
            f'          <div class="faq__answer" id="faq-answer-{idx}" role="region" aria-labelledby="faq-question-{idx}">\n'
